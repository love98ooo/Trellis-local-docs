...
| **Command**   | `/trellis:finish-work` | When recording progress or completing work                 | Archive task + record session in journal                                                 |
| **Command**   | `/trellis:continue`    | Manual                                                     | Advance the current task to its next step — you don't memorize the workflow (see §1.2.6) |
| **Skill**     | `trellis-brainstorm`   | Resolve material requirements                              | Clarify requirements, inspect evidence, draft planning artifacts                         |
...
#### 1.2.1 Install the fork version
...
#### 1.2.3 `trellis-local update`: Sync the project to the CLI
...
```bash
trellis-local update               # sync the project to the local CLI's templates
trellis-local update --dry-run     # preview changes without applying them
trellis-local update --migrate     # also apply file migrations (renames / relocations / deletes)
```

`trellis-local update` only touches files you haven't modified — your customizations stay intact, and a timestamped backup is created before any change.

<Warning>
  If the update output ends with **`MIGRATION REQUIRED`** (breaking changes between your project's
  version and the CLI, e.g. `0.4.0 → 0.6.5`), run `trellis-local update --migrate`. Without
  `--migrate`, files renamed or relocated by breaking releases are **not** moved — your project
  keeps the stale old paths alongside the new templates. Use `--dry-run` to preview what `--migrate`
  will do.
</Warning>
...
2. Run `get_context.py` to surface worktree context, git status, active tasks.
3. Read spec indexes (per relevant package in a monorepo).
4. Report context and ask what you want to work on.

Follow the current worktree’s `.trellis/workflow.md`: execute small tasks directly and record plans for complex work when useful. Ask only unresolved questions that materially affect scope, acceptance, or risk. Reuse authorization for the same scope while preserving explicit safety approvals and repository delivery requirements.
...
TASK_DIR=$(./.trellis/scripts/task.py create "Add user login" \
  --slug user-login \          # Directory name suffix (optional, auto-slugifies otherwise) \           # Assignee (optional)
...
<Note>
  ...
  status pairs), maintained only when needed. Don't confuse the two.
</Note>
...
./.trellis/scripts/task.py list            # Only your own
...
| `after_finish`  | `task.py finish` clears the current session task | Record local progress                  |
...
#### 3.1.1 Default layout from `trellis-local init`
