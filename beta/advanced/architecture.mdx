...
- **Team-level layer**: git-tracked workflow/spec/task files plus personal worktree workspace memory, so multiple people and multiple AI tools operate against the same conventions.
...
| Workspace memory        | Per-developer journals under `.trellis/workspace/`.                                                   |
...
| Developer memory                   | Workspace module        | `.trellis/workspace/journal-*.md`                                                                                              |
...
| Codex                         | Codex                                               | `AGENTS.md`; `UserPromptSubmit` can inject a no-task workflow reminder | Optional hook when enabled (0.129+ also requires `/hooks` review) | Inline mode reads artifacts through skills; sub-agent mode uses pull-based preludes.         |
...
| Workspace memory       | `.trellis/workspace/`        | Per-developer journals and indexes.                                                |
...
Stable team rules belong in `.trellis/spec/`. Task-specific facts belong in the task directory. Session notes belong in `.trellis/workspace/`.
