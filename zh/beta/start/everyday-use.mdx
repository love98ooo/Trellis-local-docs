...
| **Command** | `/trellis:finish-work` | 工作完成并通过验收后                    | 归档 task + 写入 session journal                                   |
| **Command** | `/trellis:continue`    | 手动                                    | 当前任务内推进下一步工作流，用户不用记 workflow（详见 1.2.6 小节） |
| **Skill**   | `trellis-brainstorm`   | 需要澄清复杂需求时触发                  | 澄清需求、查证据、起草 planning artifacts                          |
...
在 fork 源仓库检出并验证目标版本后，运行 `pnpm local:install` 更新独立安装的 `trellis-local`。随后在目标 worktree 先运行 `trellis-local update --dry-run` 审阅，再按仓库要求运行 `trellis-local update`；需要文件迁移时使用 `--migrate`。

#### 1.2.3 `trellis-local update`：把项目同步到 CLI 版本

升级完 CLI 后，在 Trellis 项目里运行。它把 `.trellis/` 模板和平台文件（hook、skill、command）同步到本地 CLI 的版本：

```bash
trellis-local update               # 把项目同步到本地 CLI 的模板版本
trellis-local update --dry-run     # 只预览改动，不实际执行
trellis-local update --migrate     # 同时执行文件迁移（改名 / 挪位置 / 删除）
```

`trellis-local update` 只碰你没改过的文件——自定义内容不会丢，改动前还会自动创建带时间戳的备份。

<Warning>
  如果 update 输出最后出现 **`MIGRATION REQUIRED`**（项目版本和 CLI 之间存在 breaking 变更，比如
  `0.4.0 → 0.6.5`），必须运行 `trellis-local update --migrate`。不加 `--migrate` 的话，breaking
...
2. 跑 `get_context.py` 拿到worktree 上下文、git 状态、活跃任务。
3. 读 spec 索引（monorepo 场景下按 package 读）。
4. 汇报上下文并询问你想做什么。

任务分类和授权遵循当前 worktree 的 `.trellis/workflow.md`：小任务直接执行，复杂任务按需记录计划；仅询问实质影响范围、验收或风险且无法从现有证据确定的问题。同一范围授权不重复询问，明确安全审批和目标仓库交付要求仍然有效。
...
TASK_DIR=$(./.trellis/scripts/task.py create "新增用户登录" \
  --slug user-login \          # 目录名后缀（可选，不写自动生成） \           # 负责人（可选）
...
./.trellis/scripts/task.py list            # 只看自己的
