...
里面拉取一套 `.trellis/spec/` 起点。个人项目有很多仓库、但希望它们从同一套工程约定开始时，就该用这个。
...
里面的文件夹名是项目约定，不是 Trellis 强制要求。`frontend/`、`backend/`、`shared/`、`guides/`
...
安装后，目标项目拥有自己的 `.trellis/spec/`。模板只是起点，不是一个会被远程同步的 wiki。个人项目应该继续编辑项目里的
spec，让它反映当前仓库真实情况。
...
把 template id 当成个人项目内部的公开 API。需要破坏性重写时，发一个新 id，比如 `web-app-v2`；或者让旧行为固定到
某个 Git ref：
