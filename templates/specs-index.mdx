...
<Info>
  **Specs are meant to be customized.** Trellis ships with empty spec templates by default — they
  are placeholders for *your* project's conventions. Every project's stack, patterns, and quality
  bar are different, so the specs you write should reflect your actual codebase, not generic best
  practices. Templates from the marketplace give you a head start, but always tailor them to your
  project.
</Info>
...
trellis init --registry https://github.com/your-org/your-spec-templates --cursor
```
...
<Tip>
  You don't have to fill every spec file at once. Start with the areas that matter most to your
  project, then expand over time. The bootstrap task created by `trellis init` will guide you
  through the initial fill.
</Tip>
